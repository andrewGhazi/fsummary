...
get_ch1 = function(split_chains, variables) {
  # Get the first row of the covariance
...
get_acov_means = function(split_chains, ch1_by_chain, variables, n_cov, offset, chain_info) {
  ...
  .cov_head = function(chain_dt, ch1 = ch1, n_cov=n_cov, offset=offset, f = cov_head, f2 = .check_for_nans) {
  ...
  everywhere(library(collapse))
  # everywhere(Rcpp::sourceCpp("~/fsummary/src/matrix_acov.cpp")) # nope

  m = mirai_map(list(split_chains, ch1_by_chain),
                .cov_head,
                .args = list(n_cov = minn, offset = mino, f = cov_head, f2 = .check_for_nans))
